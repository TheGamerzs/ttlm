...
it('calculates a water run', function () {
    ...
    $calc = new \App\TT\PickupRunCalculator(300);
...
it('calculates sawdust', function () {
    ...
    $calc = new \App\TT\PickupRunCalculator(60);
...
//        ],
//        'baseWeight' => 130
    $calc = new \App\TT\PickupRunCalculator(130);
    $calc->addNeededCount('scrap_copper', 16);
    expect($calc->getRunCalculations()['electronics'])->toBe(2);

    $calc = new \App\TT\PickupRunCalculator(130);
    $calc->addNeededCount('scrap_plastic', 25);
    expect($calc->getRunCalculations()['electronics'])->toBe(3);

    $calc = new \App\TT\PickupRunCalculator(130);
    $calc->addNeededCount('scrap_gold', 4);
    expect($calc->getRunCalculations()['electronics'])->toBe(4);
...
test('easy calculation method using electronics run based on multiple needs', function () {
    ...
    expect($calc->getRunCalculations()['electronics'])->toBe(5);
